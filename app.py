...
filtro_puntual.add_command(label="Identidad", command=lambda : (write_image(FP_Iden(imagen)), show_confirmacion2()))
filtro_puntual.add_command(label="Negativo", command=lambda : (write_image(FP_Neg(imagen)), show_confirmacion2()))
filtro_puntual.add_command(label="Grises", command=lambda : (write_image(FP_Grises(imagen)), show_confirmacion2()))

binario_threshold = [i for i in range(0, 256, 51)]

binario = Menu(filtro_puntual, tearoff=0)
for i in binario_threshold:
    binario.add_command(label=f'Umbral: {i}', command=lambda: (write_image(FP_Bin(imagen, args=i)), show_confirmacion2()))

binario_inverso = Menu(filtro_puntual, tearoff=0)
for i in binario_threshold:
    binario_inverso.add_command(label=f'Umbral: {i}', command=lambda: (write_image(FP_BinInv(imagen, args=i)), show_confirmacion2()))

logarit_m = [i for i in range(1, 11)]

logaritmico = Menu(filtro_puntual, tearoff=0)
for i in logarit_m:
    logaritmico.add_command(label=f'Alfa: {i}', command=lambda: (write_image(FP_Log(imagen, args=i)), show_confirmacion2()))

gamma_m = [i/4 for i in range(1, 11)]

gamma = Menu(filtro_puntual, tearoff=0)
for i in gamma_m:
    gamma.add_command(label=f'Gamma: {i}', command=lambda: (write_image(FP_gamma(imagen, args=i)), show_confirmacion2()))

expa = Menu(filtro_puntual, tearoff=0)
for i in binario_threshold:
    expa.add_command(label=f'Alfa: {i}', command=lambda: (write_image(FP_Exp_Acl(imagen, args=i)), show_confirmacion2()))


expo_a = [i/4 for i in range(1, 11)]
expo = Menu(filtro_puntual, tearoff=0)
for i in expo_a:
    expo.add_command(label=f'Alfa: {i}', command=lambda: (write_image(FP_Exp_Osc(imagen, args=i)), show_confirmacion2()))

filtro_puntual.add_cascade(label="Gamma", menu=gamma)
filtro_puntual.add_command(label="Rango Dinámico", command=lambda: (write_image(FP_Log(imagen)), show_confirmacion2()))
filtro_puntual.add_cascade(label="Rango Dinámico parametrizado", menu=logaritmico)
filtro_puntual.add_command(label="Seno", command=lambda: (write_image(FP_Seno(imagen)), show_confirmacion2()))
filtro_puntual.add_command(label="Coseno", command=lambda: (write_image(FP_Coseno(imagen)), show_confirmacion2()))
